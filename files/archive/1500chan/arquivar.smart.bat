...
for /F %%A in ('curl -b cookies.txt -s https://1500chan.org/b/threads.json ^| jq .[].threads[].no') do (
	::curl -s https://1500chan.org/b/res/%%A.json | jq [.posts[].com_nomarkup] | gzip -f -9>threads\%%A.json.gz
	curl -b cookies.txt -s https://1500chan.org/b/res/%%A.json | jq [.posts[].com_nomarkup] >%temp%\%%A.json 2> nul
